...
for (const label of taxonomyMatches) {
  if (!label.endsWith('-*') && !labelsYaml.includes(`- name: '${label}'`)) {
    failures.push(`Label '${label}' documented in issue-taxonomy.md is missing from .github/labels.yml`);
  }
}

const templateFiles = listFiles().filter(file => file.startsWith('.github/ISSUE_TEMPLATE/') && file.endsWith('.yml'));
